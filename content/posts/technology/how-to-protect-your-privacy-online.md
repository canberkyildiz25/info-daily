...
- Using email aliasing services (SimpleLogin, AnonAddy) that generate forwarding addresses — you can delete them if a company starts spamming you or gets breached

## Layer 3: Phone Privacy

Your smartphone is a comprehensive tracking device. Steps to reduce this:

**Location permissions**: Review every app's location permission. Most apps need location exactly never, or only when actively in use. Go to Settings → Privacy → Location Services and audit each app.

**App permissions generally**: Microphone, camera, contacts, photos — grant these only to apps that genuinely need them, not as a default.

**Limit ad tracking**: 
- iOS: Settings → Privacy → Tracking → disable "Allow Apps to Request to Track"
- Android: Settings → Privacy → Ads → opt out of personalized ads

**Consider using Signal** for messaging instead of SMS. Signal provides end-to-end encryption so only the sender and recipient can read messages — not the carrier, not Signal, not anyone else.
...
- Mullvad (most private — accepts anonymous cash payments, no email required to sign up)
- ProtonVPN (Swiss-based, strong no-logs policy, free tier available)
- ExpressVPN (fast, good track record)

Avoid free VPNs — many monetize by selling your browsing data, which defeats the purpose entirely.
...
1. **Change your default search to DuckDuckGo** — 2 minutes
2. **Install uBlock Origin** in your browser — 2 minutes
3. **Audit location permissions** on your phone — 10 minutes
4. **Enable 2FA on your email account** — 5 minutes
5. **Switch to Signal** for sensitive conversations — 5 minutes

Each step meaningfully reduces your exposure. Perfect privacy is unachievable with modern technology, but you can make mass data collection about you far more difficult — and that matters.
